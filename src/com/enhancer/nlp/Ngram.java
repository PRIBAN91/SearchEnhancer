...
public class Ngram {
	...
	public double numTrainingNgrams; // The size of the training set (#
										// non-distinct words)
	public HashMap<Double, Double> numberOfNgramsWithCount; // The number of
															// ngrams that occur
															// x times
	 ...
	 * "(2) Test file\n(3) an integer n > 1"); System.exit(1); } NgramParser p =
	 * new NgramParser(args[0], true); HashSet<String> set = p.parse();
	 * 
	 * Ngram n = new Ngram(set, Integer.parseInt(args[2])); n.train();
	 * 
	 * System.out.println("Done training.");
	 * 
	 * NgramParser test = new NgramParser(args[1], true); HashSet<String>
	 * testset = test.parse(); System.out.println("Perplexity of the test set: "
	 * + n.perplexity(testset));
	 * 
	 * System.exit(0); System.out.println("Press enter for generated sentences."
	 * ); BufferedReader br = new BufferedReader(new
	 * InputStreamReader(System.in)); while (true) { try { br.readLine(); }
	 * catch (IOException ioe) { System.out.println("IO error!");
	 * System.exit(1); } System.out.println(n.getSentence()); } }
	 */
	...
	public void train() {
		...
		for (String sample : samples) {
			...
			for (String word : sampleWords) {
				...
				// Insert the words into the counter and receive count for this
				// ngram
				double countForNgram = ngc.insert(nWords);

				// Decrement the number of ngrams with old countForNgram for
				// gt-smoothing
				if (countForNgram != 1.0) {
					numberOfNgramsWithCount.put(countForNgram - 1,
							numberOfNgramsWithCount.get(countForNgram - 1) - 1.0);
				}
				// Increment the number of ngrams with the new countForNgram for
				// gt-smoothing
				if (!numberOfNgramsWithCount.containsKey(countForNgram)) {
					numberOfNgramsWithCount.put(countForNgram, 1.0);
				} else {
					numberOfNgramsWithCount.put(countForNgram, numberOfNgramsWithCount.get(countForNgram) + 1.0);
				}
			}
		}

		// Set the vocab size so we don't have to call wordSet.size() more than
		// once
	...
	public String getSentence() {
		...
		// This is simply to indicate that the last symbol what we are trying to
		// figure out
		words[n - 1] = "???";

		// While we have not reached the end of the sentence and it's of
		// reasonable (400 chars) length
	...
	public double perplexity(Set<String> testSamples) {
		...
		Stack<Double> probabilities = new Stack<Double>(); // collection of
															// probabilities to
															// multiply
